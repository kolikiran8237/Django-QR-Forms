...
def qreventmanager_dwn(request, id):
    qr = QRevent.objects.get(pk=id)
    s = 'http://krishnabharambe.pythonanywhere.com/dashboard/client/' + str(qr.id)
...
def clientpay(request,eid,cid):
    if request.method == 'POST':
        ...
        param_dict = {
                ...
                'WEBSITE': 'WEBSTAGING',
                'CHANNEL_ID': 'WEB',
                'CALLBACK_URL':'http://krishnabharambe.pythonanywhere.com/dashboard/payconfirm/',
    ...
    if request.method == 'POST':
        if 'sentmail' in request.POST:
            ...
            if QRE.enablepayment == '1':
                ...
                message =  render_to_string('dashboard/printpage.html',{'QRE':QRE,'QRF':QRF,'EPS':EPS})
                email_from = settings.EMAIL_HOST_USER
                recipient_list = [QRF.email]
                ...
                EPS = Eventpay.objects.filter(clientid=cid)
                subject = 'Thank you for registering'
                message =  render_to_string('dashboard/printpage.html',{'QRE':QRE,'QRF':QRF})
                email_from = settings.EMAIL_HOST_USER
                recipient_list = [QRF.email]
                email = EmailMessage( subject, message, email_from, recipient_list )
                email.content_subtype = "html" # this is the crucial part
        ...
        if 'resentmail' in request.POST:
            QRF = QRForm.objects.get(pk=cid)
            QRE = QRevent.objects.get(pk=eid)
            QRF.mailsent = str(int(QRF.mailsent) + 1)
            QRF.save()
            # sent mail
            if QRE.enablepayment == '1':
                EPS = Eventpay.objects.filter(clientid=cid)
